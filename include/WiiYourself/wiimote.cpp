...
//Sleep(500);
			...
			//memcpy(&UniqueID, &CalibrationInfo, sizeof(CalibrationInfo));//fails with more than one wiimote 85% of the time -BSAlberto
			static DWORD ID = 1;
			UniqueID = ID;
			ID++;
...
int wiimote::ParseInput (BYTE* buff)
	...
	// lock our internal state (so RefreshState() is blocked until we're RE_DONE
